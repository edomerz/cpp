...
{
	other_.IncRefBuf(); // first inc other, handles self assignment with only 1 owner
...
{
	// create proxy to handle
	// char c = s1[3]; or
	// s1[3] = 'a';
	// Proxy ret(*this, pos_);// changed to the below
...
void RCString::SetStr(const String& str_)
{
	m_buf->m_string = str_;
}

String& RCString::GetStr()
{
	return(m_buf->m_string);
}
